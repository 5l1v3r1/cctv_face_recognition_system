...
def grabFrame():
    while (True):
        ...
        #If Frame Grab Successfull, store it it frames_list.
        if ret:
...
def recogniseFace():
    while (True):
        #If frames_list is greater than or equal to 1.
        if len(frames_list)>=1:
            #Gettign First Frame from list
            frame=frames_list[0]
...
#===================================
# Sequential Programing Starts Here
#===================================

#Train Algorithm on Known Faces
doTraining("known_faces_m/faces_list.csv")

#Start Grabing Frames
frame_thread=threading.Thread(target=grabFrame)
frame_thread.start()
#Start Processing the Frames
FaceRecog_thread=threading.Thread(target=recogniseFace)
